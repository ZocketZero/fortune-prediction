  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 animate-fadeIn pb-12">
      ...
      <div className="text-center space-y-3 relative">
        ...
        <h1 className="text-3xl sm:text-5xl font-black gold-gradient-text tracking-tight">
      ...
      {blessingResult ? (
        <div className="space-y-6 max-w-3xl mx-auto animate-fadeIn">
          ...
          <div className="relative rounded-3xl p-6 sm:p-10 border border-amber-500/40 bg-gradient-to-b from-slate-900/95 via-slate-950/95 to-slate-950/95 shadow-2xl shadow-amber-500/10 backdrop-blur-xl overflow-hidden">
            {/* Background Aura */}
            ...
            {/* Top Deity Badge */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-6 border-b border-amber-500/20">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-2xl sm:text-3xl font-extrabold text-amber-200 mt-1">
            ...
            <div className="my-6 space-y-4">
              <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
                <div className="flex items-center justify-between text-xs text-slate-400 mb-1.5">
                  ...
                  <span className="text-slate-500">ถวาย: {blessingResult.offering}</span>
                </div>
                <p className="text-base sm:text-lg text-slate-100 font-medium leading-relaxed italic bg-amber-950/20 p-3 rounded-xl border-l-4 border-amber-400">
                  "{blessingResult.userWish}"
                </p>
              </div>

              {/* Divine Blessing Box */}
              <div className="p-6 rounded-2xl bg-gradient-to-br from-amber-500/15 via-purple-900/20 to-slate-900/90 border border-amber-500/40 text-center space-y-3 shadow-inner">
                ...
                <p className="text-lg text-amber-300/95 font-semibold font-cinzel italic">
                  "{blessingResult.blessing.verse}"
                </p>
              ...
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
                ...
                <p className="text-sm text-amber-200/90 whitespace-pre-line font-medium leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4 border-t border-slate-800">
              ...
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-extrabold text-sm transition-all cursor-pointer shadow-lg shadow-amber-500/20"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span className="text-[10px] px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 shrink-0">
                          {deity.category.split('&')[0]}
                        </span>
                      </div>
                      <p className="text-xs text-amber-200/70 truncate mt-0.5">{deity.subTitle}</p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {deity.highlightAspects.slice(0, 2).map((aspect, idx) => (
                          <span
                            key={idx}
                            className="text-[10px] px-1.5 py-0.5 rounded bg-slate-800/80 text-slate-300"
                          >
                            • {aspect}
                          </span>
                        ))}
          ...
          <div className="lg:col-span-7 space-y-6">
            {/* Deity Header Card */}
            <div className="p-6 rounded-3xl bg-slate-950/80 border border-amber-500/30 space-y-4 shadow-xl relative overflow-hidden">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 via-amber-500 to-orange-600 flex items-center justify-center text-3xl shadow-lg shadow-amber-500/20">
                    {selectedDeity.avatarIcon}
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-amber-200">{selectedDeity.name}</h3>
                    <p className="text-xs text-amber-300/80">{selectedDeity.title}</p>
                  </div>
                </div>
                <span className="text-xs px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 font-medium self-start sm:self-auto">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-400 pt-1">
                <span className="text-amber-400 font-medium">เครื่องบูชาแนะนำ:</span>
                {selectedDeity.recommendedOfferings.map((item, idx) => (
                  <span key={idx} className="px-2 py-0.5 rounded-full bg-slate-900 text-slate-300 border border-slate-800 text-[11px]">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <form onSubmit={handleStartPrayer} className="p-6 rounded-3xl bg-slate-950/90 border border-amber-500/30 space-y-5 shadow-2xl">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-amber-400 transition-colors placeholder:text-slate-600"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500 font-normal">ขอเรื่องที่ดีงามและไม่เบียดเบียนผู้อื่น</span>
                </label>
                <textarea
                  rows={4}
                  required
                  ...
                  className="w-full p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-amber-400 transition-colors leading-relaxed placeholder:text-slate-600"
                />
              </div>

              {/* Suggested Wishes */}
              <div className="space-y-2">
                <p className="text-xs text-amber-300/80 font-medium">ตัวอย่างคำอธิษฐานยอดนิยม (คลิกเพื่อเลือก):</p>
                <div className="flex flex-col gap-1.5">
                  {selectedDeity.suggestedWishes.map((wish, idx) => (
                    <button
                      ...
                      className="text-left text-xs p-2.5 rounded-lg bg-slate-900/60 hover:bg-slate-800/80 text-slate-300 border border-slate-800/60 hover:border-amber-500/30 transition-all cursor-pointer truncate"
                    >
                      ✨ {wish}
                    </button>
                  ))}
              ...
              >
                {isPraying ? (
                  <>
                    <Flame className="w-5 h-5 animate-bounce text-amber-950" />
