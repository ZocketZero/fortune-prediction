...
export const EncyclopediaPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn">
      {/* Banner / Header */}
      <div className="text-center max-w-2xl mx-auto space-y-2.5 sm:space-y-3 py-1 sm:py-2">
        <span className="inline-flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 rounded-full text-[11px] sm:text-xs font-bold bg-purple-950/90 border border-purple-500/40 text-purple-300 shadow-md">
        ...
        <p className="text-xs sm:text-sm text-slate-300/80 leading-relaxed px-2">
          รวมความหมาย สัญลักษณ์ ธาตุประจำไพ่ และคำทำนายเจาะลึกของไพ่ชุดใหญ่ ๒๒ ใบ
        </p>
      </div>

      {/* Filters & Search Bar */}
      <div className="glass-panel rounded-2xl p-3.5 sm:p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 shadow-xl">
        {/* Search */}
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 sm:py-2.5 rounded-xl bg-slate-900/90 border border-purple-900/40 focus:border-amber-400 focus:outline-none text-slate-100 text-xs sm:text-sm placeholder:text-slate-500 transition-colors"
          />
        </div>

        {/* Element Filter */}
        <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0 scrollbar-none">
          <span className="text-xs text-slate-400 flex items-center gap-1 mr-1 shrink-0">
          ...
          {elements.map((elem) => (
            <button
              ...
              className={`px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer shrink-0 active:scale-95 ${
      ...
      {/* Card Grid */}
      <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2.5 sm:gap-4">
        {filteredCards.map((card: TarotCard) => (
          <button
            key={card.id}
            onClick={() => setSelectedCard(card)}
            className="glass-panel hover:border-amber-400/80 rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col items-center transition-all duration-300 cursor-pointer active:scale-95 group text-center"
          >
            <div
              className="w-16 h-24 sm:w-20 sm:h-32 rounded-xl p-1 border shadow-lg mb-2 sm:mb-3 bg-slate-950 group-hover:border-amber-400 transition-colors overflow-hidden"
      ...
      {selectedCard && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
          <div className="bg-slate-950 border border-purple-500/40 rounded-2xl sm:rounded-3xl max-w-2xl w-full max-h-[88vh] overflow-y-auto p-4 sm:p-8 relative shadow-2xl space-y-4 sm:space-y-6">
            <button
              onClick={() => setSelectedCard(null)}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-slate-900 border border-purple-800 text-slate-400 hover:text-white flex items-center justify-center cursor-pointer transition-colors"
            >
              ✕
            </button>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-center sm:items-start text-center sm:text-left">
              <div
                className="w-32 h-48 sm:w-40 sm:h-64 rounded-2xl p-1.5 border-2 shadow-2xl shrink-0 overflow-hidden bg-slate-950"
                style={{
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <h3 className="text-xl sm:text-3xl font-extrabold gold-gradient-text">{selectedCard.nameTh}</h3>
                <p className="text-xs text-purple-300 font-mono font-medium">{selectedCard.nameEn}</p>
                <div className="flex flex-wrap gap-1 sm:gap-1.5 justify-center sm:justify-start pt-1">
                  {selectedCard.keywords.map((kw: string, i: number) => (
                    <span key={i} className="text-[11px] sm:text-xs bg-purple-950 text-purple-200 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full border border-purple-800/60 font-medium">
                      #{kw}
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-purple-900/40">
              <h4 className="font-bold text-amber-300 flex items-center justify-center sm:justify-start gap-2 text-xs sm:text-sm">
                <BookOpen className="w-4 h-4 text-amber-400" /> คำทำนายตัวอย่างประจำวัน:
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 text-xs">
                <div className="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl border border-purple-900/40">
              ...
              <div className="bg-slate-900/90 p-3.5 sm:p-4 rounded-xl border border-purple-900/40 text-xs space-y-1.5 sm:space-y-2">
              ...
              {/* Advice */}
              <div className="p-3 sm:p-3.5 rounded-xl bg-purple-950/40 border border-purple-800/40 text-xs">
                <span className="font-bold text-amber-400 flex items-center gap-1.5 mb-1">
                  <Shield className="w-3.5 h-3.5 text-amber-400 shrink-0" /> คำแนะนำเสริมดวงชะตา:
                </span>
